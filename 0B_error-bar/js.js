...
function computeLinearLayout(data, size) {
  ...
  const paddingTop = 72; const paddingRight = 72; const paddingLeft = yLabelText ? Math.max(72, Math.ceil(yTickX + maxYTickWidth + 24)) : 72; const paddingBottom = xLabelText ? 106 : 72;
  const plotWidth = size - paddingLeft - paddingRight; const plotHeight = size - paddingTop - paddingBottom; const toX = (value) => paddingLeft + ((value - xMin) / xRange) * plotWidth; const toY = (value) => size - paddingBottom - ((value - yMin) / yRange) * plotHeight;
  return { xMin, yMin, xRange, yRange, xLimit, yLimit, exponent, divisions, paddingTop, paddingRight, paddingLeft, paddingBottom, toX, toY, labelFontSize, yTickX, yTitleX };
}
function draw(data, size = canvas.clientWidth) { context.clearRect(0, 0, size, size); context.fillStyle = '#fcfdfd'; context.fillRect(0, 0, size, size);
  const { xMin, yMin, xRange, yRange, xLimit, exponent, divisions, paddingTop, paddingRight, paddingLeft, paddingBottom, toX, toY, labelFontSize, yTickX, yTitleX } = computeLinearLayout(data, size);
  chartStatus.textContent = getQuadrantStatus(data);
